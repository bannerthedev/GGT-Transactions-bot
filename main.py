...
ASSIGNMENTS_CHANNEL_ID = 1499238872951947276
TRANSACTIONS_CHANNEL_ID = 1492676697953140876
MATCH_SCORES_CHANNEL_ID = 1499237515884757132
# ----------------------------------------------------
...
class InviteUserSelect(discord.ui.UserSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # No Free Agent requirement – anyone can be invited
...
class RosterSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        lines = [
            ...
            f"co-captain: {cocap.mention if cocap else 'None'}",
            "players:"
        ]
        if players:
...
async def code(interaction: discord.Interaction, team1: discord.Role, team2: discord.Role):
    ...
    # Generate random code: GGT + 4 digits
...
@discord.app_commands.describe(
    teams="Example: Team A vs Team B",
    winner_team="Winning team name",
    score="Final score (e.g. 3-1)",
    advancing_team="Team that moves on",
    next_stage="What they move into (e.g. 'bracket round', 'round of seeding')"
)
...
async def captain_panel(interaction: discord.Interaction):
    ...
    # ONLY Captain | TeamName (no Co-Captain | here)
    team_name = None
    for r in member.roles:
        lower = r.name.lower()
    ...
    embed = discord.Embed(
        title=f"GGT Captain Panel – {team_name}",
        description=desc,
    ...
    embed.set_footer(text="GGT Transactions Bot")
...
async def co_captain_panel(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        title=f"GGT Co-Captain Panel – {team_name}",
        description=desc,
...
@bot.tree.command(
    guild=discord.Object(id=GUILD_ID),
    name="info",
    description="Show information about the GGT Transactions Bot commands"
)
async def info(interaction: discord.Interaction):
    ...
    embed.set_footer(text="GGT Transactions Bot • created by banner")
